...
class ChannelSELayer3D(nn.Module):
    ...
    def __init__(self, num_channels, reduction_ratio=2):
        ...
        if num_channels==1: reduction_ratio=1
...
class ChannelSpatialSELayer3D(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        :return: output_tensor
        """
        c_se = self.cSE(input_tensor)
        s_se = self.sSE(input_tensor)
        output_tensor = torch.max(c_se, s_se)
        return output_tensor
...
class ProjectExciteLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        # Excitation:
        c = self.conv_c(final_squeeze_tensor) 
        c = self.conv_cT(self.relu(c)) 
        final_squeeze_tensor = self.sigmoid(c)
